...
class retrieveRegulatorDetail extends Component {
	constructor(props) {
		super(props)
		if(config.back.url[config.back.url.length-1] !== '/retrieveRegulatorDetail/'+this.props.match.params.idx){
			config.back.url.push('/retrieveRegulatorDetail/'+this.props.match.params.idx);
		}
	...
	fetchDetail = async () => { 
		if(config.detail_file.index !== '' ){
			if(config.detail_file.name!==''){
				...
				config.table.attach3=result.pgovNo;
			...
			<div className="contents">
						...
						<div className="box info">
							<table>
								...
								<tbody>
									...
									<tr>
										<th className="important"><span>*</span>분해점검일</th>
				...
				<footer>
					<div className="footer_contents">
						...
						<button type="button" className="btn_file" onClick={() => config.AttachMove(this.props,'retrieveRegulator','압력정압기')}><img alt="" src={icon2} width="15" height="15" />첨부파일</button>
					</div>
				</footer>
			</div>
		);
...
export default retrieveRegulatorDetail;