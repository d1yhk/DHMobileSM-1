...
class retrieveCoatdamage extends Component {
	...
	lists = (result,stype,search_type) => {
		...
		if( result.length <= 0 ){
			$(".grp_coastamage_list tbody").html('<tr><td colspan="10">검색 결과가 없습니다.</td></tr>');
		}
	...
	fetchSearch = async (search_type) => { 
		...
		try {
			...
			if(common[0].data.code > 0){
				...
				$(".message").html("&nbsp;");
				$(".grp_coastamage_list tbody").html('<tr><td colspan="10">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
			}
		} catch(err){
			$(".message").html("&nbsp;");
			$(".grp_coastamage_list tbody").html('<tr><td colspan="10">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
	...
	componentDidMount() {
		...
		$(document).on("click",".grp_coastamage_list tr",function(){
			...
			config.table.p1="피복손상부";
			config.table.p2=$(this).find("td:eq(0)").attr("data-id");


			config.table.attach1="피복손상부";
			...
			<div className="contents">
				<div className="list">
					<div className="tab tab3">
						<Link to="/retrieveCoatdamage" className="active"><span>피복손상부</span></Link>
