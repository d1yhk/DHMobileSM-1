...
class retrieveAutoPeriodicInsp extends Component {
	constructor(props) {
		super(props);
		this.page = {change : 0,num: 0, type:0,count:0, page:0, total:0 };
		if(config.back.url[config.back.url.length-1] !== '/retrieveAutoPeriodicInsp'){
			config.back.url.push('/retrieveAutoPeriodicInsp');
		}
		
		if(config.table_name.file !== "retrieveAutoPeriodicInsp"){
	...
	lists = (result,stype,search_type) => {
		...
		for(var count = 0; count < result.length; count++){
			...
			td = td + '<tr>';
				...
				td = td + '<td>'+((result[count]['qtyGov']) ? result[count]['qtyGov'] : '')+'</td>';
			td = td + '</tr>';
		}
		if(search_type === 1){
			$(".grp_period_list tbody").html(td);
		}else{
			$(".grp_period_list tbody").append(td);
		}
			
		if( result.length <= 0 ){
			$(".grp_period_list tbody").html('<tr><td colspan="8">검색 결과가 없습니다.</td></tr>');
		}
	...
	fetchSearch = async (search_type) => { 	
		...
		try {
			const common = await Promise.all([ 
				...
				service.retrieveAutoPeriodicInspList(inspGubun,dtInspPlnFr,dtInspPlnTo,nmFclt,txAddr,(this.page.page * 100),100)
			]);
			if(common[0].data.code > 0){
				...
				if(this.page.type === 0){
					this.page.num = $(".grp_period_list tbody").height() / 2;
					this.page.type=1;
				...
				$(".message").html("&nbsp;");
				$(".grp_period_list tbody").html('<tr><td colspan="8">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
			}
		} catch(err){
				$(".message").html("&nbsp;");
				$(".grp_period_list tbody").html('<tr><td colspan="8">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
	...
	fetchDetail = () => {
		if(config.detail_file.index === ""){
			...
			this.props.history.push('/retrieveAutoPeriodicInspDetail/'+cdFclt+"/"+idInsp+"/"+idInspType+"/"+inspGubunNm);
	...
	componentDidMount() {
		...
		config.table_name.file = "retrieveAutoPeriodicInsp"


		config.fetchCommon("정압기","점검유형","inspGubun","자율정기");
		...
		if(config.grpifm.list.length > 0 ){
			...
			$(".grp_period_list tr:eq("+config.grpifm.select_no+")").css({"background-color":"#eee"});
			setTimeout(function(){
				$(".grp_period_list").scrollTop( config.grpifm.scroll);
			},500);
		...
		$(".grp_period_list").scroll(function(){
			
			if((t.page.total - (100 * t.page.page)) >= 100){
				if($(".grp_period_list").scrollTop() >= ($(".grp_period_list tbody").height() - t.page.num)){
					if(t.page.change === 0){
						t.page.change = 1;
						t.page.page++;
						t.fetchSearch(0);
		...
		//테이블 리스트 선택
		$(document).off("click",".grp_period_list tr");
		$(document).on("click",".grp_period_list tr",function(){

			config.grpifm.scroll = $(".grp_period_list").scrollTop();
			config.grpifm.select_no = $(this).index();
			$(".grp_period_list tr").css({"background-color":"transparent"});
			...
			<div className="contents">
				<div className="list">
					<div className="tab tab6">
						...
						<Link to="/retrieveAutoPeriodicInsp" className="active"><span>자율/정기검사</span></Link>
...
isClearable
						...
						<div className="box table">
							<div id="gridBox">
								...
								<div id="gridContainer" className="mw1000 grp_period_list">
									<table>
										<colgroup>
											<col width="120"/>
											<col width="100"/>
...
export default retrieveAutoPeriodicInsp;