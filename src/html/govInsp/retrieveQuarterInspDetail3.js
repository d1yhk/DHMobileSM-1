...
class retrieveQuarterInspDetail3 extends Component {
  ...
  constructor(props) {
    super(props);
		...
		if(config.back.url[config.back.url.length-1] !== '/retrieveQuarterInspDetail3/'+this.props.match.params.cdFclt+"/"+this.props.match.params.idInsp+"/"+this.props.match.params.idInspType){
			config.back.url.push('/retrieveQuarterInspDetail3/'+this.props.match.params.cdFclt+"/"+this.props.match.params.idInsp+"/"+this.props.match.params.idInspType);
		}
	...
	fetchDetail = async () => {
		...
		for(var count = 0; count < safety_list.length; count++){
			td = td + '<tr>';
			if(count == 0 ){
				td = td + '<td rowspan="'+safety_list.length+'">'+$(".nmFclt").html()+'</td>';
			}
			...
			td = td + '<td><input type="text" value="'+((safety_list[count].remark) ? safety_list[count].remark : '')+'" /><input type="hidden" value="'+((safety_list[count].seqSort) ? safety_list[count].seqSort : '')+'" /></td>';
			td = td + '</tr>';
		}
	...
	btnSave = async () => {
		if(window.confirm("저장 하시겠습니까?")){
			...
			$("#safty_list tbody").find("tr").each(function(){
				...
				t.safetyListReData[(id0-1)]['remark'] = $(this).find("input:eq(3)").val();
						...
						<div className="box info">
							<div className="row_box2">
								<table id="safty_list">
									<thead>
										<tr>
											<th className="text-center">시설명</th>
...
export default retrieveQuarterInspDetail3;