...
class retrieveRectifier extends Component {
	constructor(props) {
		super(props);
		this.page = {change : 0,num: 0, type:0,count:0, page:0, total:0 };
		if(config.back.url[config.back.url.length-1] !== '/retrieveRectifier'){
			config.back.url.push('/retrieveRectifier');
		}

		if(config.table_name.file !== "retrieveRectifier"){
	...
	lists = (result,stype,search_type) => {
		...
		for(var count = 0; count < result.length; count++){
			...
			td = td + '<tr>';
					...
					td = td + '<td data-fn="'+result[count]['facilNo']+'">'+((result[count]['idConst']) ? result[count]['idConst'] : '')+'</td>';
					td = td + '<td>'+((result[count]['nmConst']) ? result[count]['nmConst'] : '')+'</td>';
					td = td + '<td>'+((result[count]['facilNo']) ? result[count]['facilNo'] : '')+'</td>';
					td = td + '<td>'+((result[count]['exvtgNm']) ? result[count]['exvtgNm'] : '')+'</td>';
					td = td + '<td>'+((result[count]['statusNm']) ? result[count]['statusNm'] : '')+'</td>';
			td = td + '</tr>';
		}
		if(search_type === 1){
			$(".grp_rectifier_list tbody").html(td);
		}else{
			$(".grp_rectifier_list tbody").append(td);
		}
			
		if( result.length <= 0 ){
			$(".grp_rectifier_list tbody").html('<tr><td colspan="6">검색 결과가 없습니다.</td></tr>');
		}
	...
	fetchSearch = async (search_type) => { 
		...
		var exvtgNm = config.form_search(search_type,'exvtgNm');
		...
		try {
			const common = await Promise.all([ 
				service.retrieveRectifierList(bjdNm,idConst,nmConst,exvtgNm,status,(this.page.page * 100),100)
			]);
			if(common[0].data.code > 0){
				...
				if(this.page.type === 0){
					this.page.num = $(".grp_rectifier_list tbody").height() / 2;
					this.page.type=1;
				...
				$(".message").html("&nbsp;");
				$(".grp_rectifier_list tbody").html('<tr><td colspan="6">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
			}
		} catch(err){
				$(".message").html("&nbsp;");
				$(".grp_rectifier_list tbody").html('<tr><td colspan="6">검색 오류입니다. 관리자에게 문의하세요.</td></tr>');
	...
	componentDidMount() {
		...
		config.table_name.file = 'retrieveRectifier';
		...
		if(config.grpifm.list.length > 0 ){
			...
			$(".grp_rectifier_list tr:eq("+config.grpifm.select_no+")").css({"background-color":"#eee"});
			setTimeout(function(){
				$(".grp_rectifier_list").scrollTop( config.grpifm.scroll);
			},500);
		...
		$(".grp_rectifier_list").scroll(function(){
			
			if((t.page.total - (100 * t.page.page)) >= 100){
				if($(".grp_rectifier_list").scrollTop() >= ($(".grp_rectifier_list tbody").height() - t.page.num)){
					if(t.page.change === 0){
						t.page.change = 1;
						t.page.page++;
						t.fetchSearch(0);
		...
		//테이블 리스트 선택
		$(document).off("click",".grp_rectifier_list tr");
		$(document).on("click",".grp_rectifier_list tr",function(){
			config.grpifm.scroll = $(".grp_rectifier_list").scrollTop();
			config.grpifm.select_no = $(this).index();
			$(".grp_rectifier_list tr").css({"background-color":"transparent"});
			$(this).css({"background-color":"#eee"});
			
			config.table.p1="외부전원장치";
			config.table.p2=$(this).find("td:eq(0)").attr("data-id");



			config.table.attach1="외부전원장치";
			config.table.attach2=config.grpifm.list[config.grpifm.select_no].facilNo;
			config.table.attach3="";
			...
			<div className="contents">
				<div className="list">
					<div className="tab tab3">
						...
						<Link to="/retrieveRectifier" className="active"><span>외부전원장치</span></Link>
					...
					<div className="wrap">
						...
						<div className="box table">
							<div id="gridBox">
								...
								<div id="gridContainer" className="mw1000 grp_rectifier_list">
									<table>
										<colgroup>
											<col width="*"/>
											<col width="150"/>
				...
				<footer>
					<div className="footer_contents">
						...
						<button type="button" className="btn_detail" onClick={() => config.fetchDetail(this.props,'retrieveRectifierDetail')}><img alt="" src={icon1} width="15" height="15" />상세정보</button>
					</div>
				</footer>
...
export default retrieveRectifier;